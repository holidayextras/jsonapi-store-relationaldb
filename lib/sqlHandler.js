...
SqlStore.prototype._buildModels = function() {
  ...
  switch (self.config.dialect) {
    case 'postgres':
      ...
      break;
    case 'mssql':
      modelAttributes = new modelGenerators.Mssql(self.resourceConfig.resource, localAttributes);
      break;
    default:
...
SqlStore.prototype._generateSearchIncludes = function(relationships) {
  ...
  var searchIncludes = Object.keys(self.relations).reduce(
    function(partialSearchIncludes, relationName) {
...
SqlStore.prototype._scalarFilterElementToWhereObj = function(element) {
  ...
  if (op === '>') return { [Symbol.for('gt')]: value };
  if (op === '<') return { [Symbol.for('lt')]: value };

  var iLikeOperator = Symbol.for('like');
  if (self.sequelize.getDialect() === 'postgres') iLikeOperator = Symbol.for('iLike');
...
SqlStore.prototype._filterElementToSearchBlock = function(filterElement) {
  ...
  return { [Symbol.for('or')]: whereObjs };
};
...
SqlStore.prototype.search = function(request, callback) {
  ...
  self.baseModel.count(options).asCallback(function(ignoredErr, count) {
    ...
    self.baseModel.findAll(options).asCallback(function(err, result) {
      ...
      var records = result.map(function(i) {
        return self._fixObject(i.toJSON());
      });
          ...
          return callback(
            new SqlStoreError({
              ...
              detail:
                ...
                ' for ' +
                request.params.type +
                ' is of wrong format. (Make sure it is UUID)'
            })
          );
        }
